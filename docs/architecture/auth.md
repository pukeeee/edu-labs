...
Цей feature-slice містить всю клієнтську логіку, з якою взаємодіє користувач під час автентифікації.

-   **`ui/AuthButton.tsx`**:
    -   Клієнтський компонент, що є точкою входу в процес автентифікації. Це "розумна" кнопка з власною машиною станів (`idle`, `loading`, `success`, `error`), яка надає користувачу детальний зворотний зв'язок.
    -   Обробляє різні сценарії, як-от закриття спливаючого вікна або помилки мережі, показуючи відповідні повідомлення.
    -   Гнучко налаштовується через пропси (`variant`, `size`) та колбеки (`onSuccess`, `onError`).
    -   Ініціює процес `signInWithOAuth`, формуючи правильний URL для редіректу.

-   **`ui/AuthModal.tsx`**:
    -   Клієнтський компонент, що показує модальне вікно із закликом до входу.
    -   Його видимість керується глобально через `useAuthModalStore`, що дозволяє викликати його з будь-якої частини додатку.
    -   Має інформативний інтерфейс, що показує користувачеві переваги автентифікації (збереження прогресу, досвід, статистика).
    -   Містить компонент `AuthButton` та посилання на політику конфіденційності.

-   **`model/auth-modal.store.ts`**:
    -   Простий стор на базі **Zustand**, призначений для глобального керування станом `AuthModal`.
    -   Експортує хук `useAuthModalStore`, який надає доступ до стану `isOpen` та дій `open` і `close`.

-   **`model/use-auth.tsx`**:
    -   Центральний клієнтський хук для взаємодії з системою автентифікації. Це основний інструмент для захисту дій та компонентів на клієнті.
    -   **Надає:**
        -   `isAuthenticated`: булевий прапорець, що показує, чи авторизований користувач.
        -   `user`: об'єкт поточного користувача.
        -   `openAuthModal` / `closeAuthModal`: функції для прямого керування модальним вікном.
    -   **Ключові утиліти:**
        -   `withAuthCheck(action, options)`: Функція вищого порядку, що "обгортає" будь-яку дію. Якщо користувач не автентифікований, вона за замовчуванням відкриє модальне вікно і не виконає дію.
        -   `withAuth(Component, options)`: Компонент вищого порядку (HOC) для обгортання цілих компонентів (наприклад, сторінок). Якщо користувач не увійшов, він може відмалювати запасний UI (fallback) або виконати редірект.
